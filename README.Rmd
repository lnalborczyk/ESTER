...
[![downloads](http://cranlogs.r-pkg.org/badges/ESTER)](http://cran.rstudio.com/web/packages/ESTER/index.html)
...
This first function runs many simulated studies in which we compare two independant groups, as a function of effect size and sample size. The `nmin` argument serves to specify from which participant we want to start doing sequential testing (we usually recommand to avoid `nmin` < 10) and the `boundary` argument specifies the value of $ER_{10}$ at which the sequential testing should be terminated.
