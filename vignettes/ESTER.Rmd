...
```{r echo = TRUE, eval = TRUE, fig.align = "center", fig.height = 4, fig.width = 4}
library(magrittr)
library(ESTER)
ER <- simER(cohensd = 0.6, n = 100, nmin = 20, plot = TRUE)
```

Then, one can study the long-term properties of evidence ratios using the `distER` function.

```{r echo = TRUE, eval = TRUE, fig.align = "center", fig.height = 4, fig.width = 6}
ER <- distER(cohensd = 0.6, n = 100, nmin = 20, nSims = 100)
```

## 2. Observed data

`seqERboot` allows you to study the behavior of sequential ERs computed on an observed sample.
...
```{r echo = TRUE, eval = TRUE, fig.align = "center", fig.height = 4, fig.width = 4, warning = FALSE, results = "hide"}
data(mtcars)
mod1 <- lm(mpg ~ cyl, mtcars)
mod2 <- lm(mpg ~ cyl + disp, mtcars)
seqER(mod1, mod2, nmin = 10) %>% plot
```

`seqERboot` allows you to study the behavior of sequential ERs computed on an observed sample along with sequential ERs computed on bootstrapped samples.
...
```{r echo = TRUE, eval = TRUE, fig.align = "center", fig.height = 4, fig.width = 4, warning = FALSE, results = "hide"}
seqERboot(mod1, mod2, order_nb = 100, nmin = 10, replace = TRUE) %>% plot
```

Under the hood, `ESTER` uses the `AICcmodavg` package (available on [CRAN](https://cran.r-project.org/web/packages/AICcmodavg/index.html); Mazerolle, 2016) to compute AICc and Akaike Weights.
